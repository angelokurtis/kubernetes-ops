...
resource "kubectl_manifest" "opentelemetry_collector_metrics_scraper" {
  server_side_apply = true
  yaml_body         = yamlencode({
    apiVersion = "opentelemetry.io/v1alpha1"
    kind       = "OpenTelemetryCollector"
    metadata   = { name = "metrics-scraper", namespace = kubernetes_namespace_v1.opentelemetry.metadata[0].name }
    spec       = {
      config = file("opentelemetry-collector/scraper.yaml")
      mode   = "daemonset"
      env    = [
        {
          name  = "PROMETHEUS_PUSHGATEWAY_ENDPOINT",
...
resource "kubernetes_cluster_role_binding_v1" "pods_reader" {
  metadata {
    name = "${kubectl_manifest.opentelemetry_collector_metrics_scraper.name}-collector-pods-reader"
  }
  role_ref {
  ...
  subject {
    kind      = "ServiceAccount"
    name      = "${kubectl_manifest.opentelemetry_collector_metrics_scraper.name}-collector"
    namespace = kubectl_manifest.opentelemetry_collector_metrics_scraper.namespace
  }
}
